...
static struct wu_data wu_memory[100];
static int m_i = 0;
...
static pthread_t thread_data_reader;
...
int set_energy_counter(int argc, char ** argv)
{
	...
	if (!wu_no_data) {
		...
		//if (pthread_create (&thread_data_reader, NULL, wu_read_data,  fd))
    		//{
        	//	perror("data reader pthread_create error!");       
        	//	exit(EXIT_FAILURE);
    		//}	    
		wu_read_data(fd);                				
	}
